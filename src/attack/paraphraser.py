"""LLM paraphraser used as the adversary in the attack loop.

Supports two backends, selected by model name:
- Local Ollama (the four smaller attackers run on the host machine)
- Hosted chat-completion APIs (Hyperbolic for Llama 3.3 70B, Together AI for GPT-OSS 120B)

The class is thin on purpose: prompt construction and direction-aware example
selection live here, and the hosted-API path includes exponential backoff for
the transient errors common with serverless endpoints.
"""
...
# Cloud providers, keyed by the model name passed in by main.py
PROVIDERS = {
...
# Retry tuning for the hosted-API path
FATAL_HTTP_STATUSES = {400, 401, 402, 403, 404}
RATE_LIMIT_SLEEP_SECONDS = 150
...
class Paraphraser:
    ...
    def _build_prompt(self, text, k, original_label):
        ...
        adversarial_goal = (
            ...
            f'Each paraphrase must be within 10% of the original length (original: {word_count} words). Do not summarize or shorten it. '
    ...
    def generate(self, text, k=5, original_label=0):
        ...
        if self.model in PROVIDERS:
            content = self._call_hosted_api(prompt)
        else:
            content = self._call_ollama(prompt)
        return self._parse_candidates(content, k)

    def _call_hosted_api(self, prompt):
        provider = PROVIDERS[self.model]
        api_key = os.environ[provider["api_key_env"]]
        attempt = 0
        while True:
            attempt += 1
            sleep_for = min(2 ** min(attempt - 1, 8), MAX_BACKOFF_SECONDS)
            try:
                ...
                print(f"[attempt {attempt}] Network error: {e}; retrying in {sleep_for}s")
            ...
            try:
                ...
                print(f"[attempt {attempt}] Non-JSON response (status {response.status_code}, "
                      f"body: {response.text[:200]!r}); retrying in {sleep_for}s")
                time.sleep(sleep_for)
                continue
            ...
            # 4xx (except 429) means a misconfigured request: bail out instead of retrying forever.
            if response.status_code in FATAL_HTTP_STATUSES:
                raise RuntimeError(f"Fatal API error (status {response.status_code}): {body}")

            if response.status_code == 429:
                print(f"[attempt {attempt}] Rate limit hit; sleeping {RATE_LIMIT_SLEEP_SECONDS}s")
                time.sleep(RATE_LIMIT_SLEEP_SECONDS)
            else:
                print(f"[attempt {attempt}] Transient error (status {response.status_code}); "
    ...
    @staticmethod
    def _parse_candidates(content, k):
        """Split the raw model output into a deduplicated list of at most k candidates."""
        lines = content.strip().split("\n")
        # Strip "1." / "1)" enumeration prefixes and surrounding quotes the model sometimes adds.
